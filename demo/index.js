...
document.getElementById("sel").addEventListener("change", printMsg);
function printMsg(evt) {
  if (evt.target.value === 'Tacotron2 encoder')
    vis('Tacotron2 encoder');
  else if (evt.target.value === 'UniNet encoder')
    vis('UniNet encoder');
  else if (evt.target.value === 'UniNet decoder')
    vis('UniNet decoder');
}

function vis(model_name) {
  let data = [];
  let quadtree;
  var labFill, toneFill, fillColor, color_labFill, color_toneFill;
  var closestDatum, cur_lab = "undefined";
  var lab_selectedQ = true,
    tone_selectedQ = false;
  console.log(model_name);
  document.getElementById('model_name').textContent = model_name + ' tSNE';
  ...
  streamingLoaderWorker.postMessage(model_name + ".tsv");
